 ...
 * Guest (unauthenticated) DigiLocker flow — creates a consent URL.
 * Body: { aadhaarNumber: string, sessionId: string }
 *
 * GET /api/kyc/guest-digilocker?verification_id=xxx
 * Fetches verification status + Aadhaar document after redirect.
 */
...
export async function POST(request: NextRequest) {
  try {
    ...
    const { aadhaarNumber, sessionId } = await request.json();
    ...
    // Check if DigiLocker account exists
    let userFlow = 'signup';
    try {
...
export async function GET(request: NextRequest) {
  try {
    ...
    if (!verificationId && !referenceId) {
      return NextResponse.json({ error: 'Missing verification_id or reference_id' }, { status: 400 });
    }
