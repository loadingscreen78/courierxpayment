...
export async function POST(request: NextRequest) {
  try {
    ...
    if (!result.success) {
      console.error('[Guest OTP] Failed to send:', result.error);
      return NextResponse.json({ error: 'Failed to send verification email. Please try again.' }, { status: 500 });
    }

    console.log(`[Guest OTP] Sent to ${normalizedEmail} - ID: ${result.id}`);

    // Return the hash + expiry so the client can send it back for verification
    ...
    console.error('[Guest OTP] Error:', err?.message);
